...
function SectionHeader({ title }: { title: string }) {
  return (
    <div class="px-5 py-2 bg-zinc-50 border-y border-zinc-100">
      <h3 class="text-[11px] font-semibold text-zinc-400 uppercase tracking-wider text-center">
        {title}
      </h3>
    </div>
  );
  ...
  return (
    ...
    >
      <Tabs defaultValue="edit">
        <TabList>
          <Tab value="edit" icon={<EditIcon />}>
            Edit
          </Tab>
          <Tab value="ai" icon={<AIIcon />}>
            AI
          </Tab>
          <Tab value="presets" icon={<PresetsIcon />}>
            Presets
          </Tab>
        </TabList>
