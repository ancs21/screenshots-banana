...
function InsetSwatch({ color, onChange }: { color: string; onChange: (c: string) => void }) {
	return (
		<label class="relative cursor-pointer group shrink-0">
			<div class="relative">
				...
				<div class="absolute -top-1 -right-1 w-4 h-4 bg-white rounded-full shadow-sm border border-zinc-200 flex items-center justify-center">
					<svg class="w-2.5 h-2.5 text-zinc-400" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2">
						<path d="M6 10l4-4M8.5 4.5L11 2a2.12 2.12 0 013 3l-2.5 2.5M7.5 11.5L5 14a2.12 2.12 0 01-3-3l2.5-2.5" stroke-linecap="round" />
					</svg>
				</div>
			</div>
			<input
...
function ShadowSwatch({ color, onChange }: { color: string; onChange: (c: string) => void }) {
	return (
		<label class="relative cursor-pointer group shrink-0">
			<div class="relative">
				<div
