...
async function exportImage(options: ExportOptions = { format: 'png', withBackground: true }): Promise<string | null> {
...
const formatOptions: { format: ExportFormat; label: string; ext: string; shortcut: string }[] = [
...
export function ExportButtons() {
	...
	const handleDownload = async (format: ExportFormat, withBackground = true) => {
		setSaving(true);
	...
	const currentScale = scaleOptions.find(s => s.value === exportScale.value) || scaleOptions[1];
			...
			<Popover.Root open={scaleOpen} onOpenChange={setScaleOpen}>
				...
				<Popover.Portal>
					<Popover.Positioner side="top" sideOffset={8} className="z-50">
						<Popover.Popup
							class="bg-white rounded-lg shadow-lg border border-zinc-200 p-1 min-w-[120px]"
						>
							{scaleOptions.map((option) => (
								...
								>
									...
									<span class="text-[11px] text-zinc-400">{option.sublabel}</span>
								</button>
							))}
						</Popover.Popup>
					</Popover.Positioner>
			...
			<Popover.Root open={saveMenuOpen} onOpenChange={setSaveMenuOpen}>
				...
				<Popover.Portal>
					<Popover.Positioner side="top" sideOffset={8} align="end" className="z-50">
						<Popover.Popup
							class="bg-white rounded-xl shadow-lg border border-zinc-200 py-2 min-w-[200px]"
						>
