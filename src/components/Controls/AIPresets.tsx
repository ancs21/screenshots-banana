...
import { aiPresets, loadAIPresets, removeAIPreset, presetsLoading } from '../../store/ai';
...
function PresetCard({ preset, onApply, onDelete }: {
	preset: { id: string; name: string; prompt: string; imageData: string; type: string };
	onApply: () => void;
	onDelete: () => void;
}) {
	...
	return (
		...
		>
			...
			{/* Hover overlay */}
			<div class={`absolute inset-0 transition-all duration-200
				...
				<div class="absolute inset-0 flex flex-col justify-end p-3">
					{/* Name */}
					<p class="text-[11px] text-white font-medium mb-2 truncate" title={preset.prompt}>
						{preset.name}
					</p>
		...
		return (
			<div class="flex-1 flex flex-col items-center justify-center p-8 text-center bg-zinc-50/50">
				...
				<div class="relative mb-6">
					<div class="w-20 h-20 rounded-2xl bg-gradient-to-br from-zinc-100 to-zinc-200
				...
				<p class="text-sm text-zinc-500 max-w-[200px] leading-relaxed">
					Generate images in the AI tab and save them as presets for quick reuse.
				</p>
				...
				<div class="mt-6 flex items-center gap-2 px-4 py-2 bg-white rounded-lg border border-zinc-200 shadow-sm">
					...
					<span class="text-xs text-zinc-600">Switch to the <strong>AI</strong> tab to get started</span>
				</div>
			</div>
		);
	}
			...
			<div class="px-4 py-3 bg-white border-b border-zinc-100">
				<div class="flex items-center justify-between">
					<div>
						...
						<p class="text-[11px] text-zinc-500">{aiPresets.value.length} item{aiPresets.value.length !== 1 ? 's' : ''}</p>
					</div>
				</div>
			</div>
