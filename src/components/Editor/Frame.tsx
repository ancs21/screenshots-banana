...
function TrafficLights({
  ...
  variant?: 'default' | 'adaptive' | 'rose' | 'muted';
}) {
  if (variant === 'adaptive') {
    return (
  ...
  return (
    <div class="flex gap-2 px-4 py-3">
      <div
        ...
        style={{ backgroundColor: TRAFFIC_LIGHT_COLORS.close }}
      />
      <div
        class="w-3 h-3 rounded-full"
        style={{ backgroundColor: TRAFFIC_LIGHT_COLORS.minimize }}
      />
      <div
        class="w-3 h-3 rounded-full"
        style={{ backgroundColor: TRAFFIC_LIGHT_COLORS.maximize }}
      />
    </div>
  );
}
...
function WindowsControls({ dark = false }: { dark?: boolean }) {
  ...
  return (
    <div class="flex ml-auto h-8">
      <button
        class={`w-12 h-full flex items-center justify-center ${hoverBg} ${textColor} transition-colors`}
      >
        <svg
          class="w-3 h-3"
          viewBox="0 0 12 12"
      ...
      <button
        class={`w-12 h-full flex items-center justify-center hover:bg-red-500 hover:text-white ${textColor} transition-colors`}
      >
        <svg
          class="w-3 h-3"
          viewBox="0 0 12 12"
  ...
  if (style === 'eclipse') {
    return (
      <div class="relative pb-3">
        <div class="absolute inset-x-6 bottom-0 h-6 bg-zinc-400/40 rounded-[50%] blur-md" />
        <div
          class="relative bg-white border border-zinc-200"
          style={{ borderRadius: `${radius}px`, boxShadow: subtleShadow }}
        >
  ...
  if (style === 'shadow-back') {
    return (
      <div class="relative pb-2">
        <div class="absolute inset-x-4 top-4 bottom-0 bg-zinc-400/30 blur-xl rounded-xl" />
        <div
