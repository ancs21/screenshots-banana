...
import { isCropping, cropRegion, imageData, setImage } from '../../store/editor';
...
export function CropOverlay() {
	...
	const handlePointerDown = useCallback((e: PointerEvent, dragType: DragState['dragType']) => {
		e.preventDefault();
		e.stopPropagation();
	...
	const handlePointerMove = useCallback((e: PointerEvent) => {
		if (!dragState.isDragging || !cropRegion.value || !containerRef.current) return;
		...
		switch (dragType) {
			case 'move':
				newRegion.x = Math.max(0, Math.min(dimensions.width - startRegion.width, startRegion.x + deltaX));
				newRegion.y = Math.max(0, Math.min(dimensions.height - startRegion.height, startRegion.y + deltaY));
				break;
			case 'nw':
				newRegion.x = Math.min(startRegion.x + startRegion.width - minSize, startRegion.x + deltaX);
				newRegion.y = Math.min(startRegion.y + startRegion.height - minSize, startRegion.y + deltaY);
		...
		newRegion.width = Math.min(dimensions.width - newRegion.x, newRegion.width);
		newRegion.height = Math.min(dimensions.height - newRegion.y, newRegion.height);
	...
	const region = cropRegion.value || { x: 0, y: 0, width: dimensions.width, height: dimensions.height };
	const scaleX = containerRef.current ? containerRef.current.offsetWidth / dimensions.width : 1;
	const scaleY = containerRef.current ? containerRef.current.offsetHeight / dimensions.height : 1;
			...
			<div class="relative max-w-4xl max-h-[80vh] flex flex-col gap-4">
				...
				>
					...
					<div
						...
						style={{ height: `${(dimensions.height - region.y - region.height) * scaleY}px` }}
					/>
					{/* Dark Overlay - Left */}
					<div
