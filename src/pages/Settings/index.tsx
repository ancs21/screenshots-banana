...
import { getApiKey, setApiKey, removeApiKey, getModel, setModel, GEMINI_MODELS, type GeminiModel } from '../../lib/db';
			...
			<main class="max-w-2xl mx-auto px-6 py-8 space-y-6">
				...
				<section class="bg-white border border-stone-200 rounded-lg">
					<div class="px-5 py-4 border-b border-stone-100">
						...
						<p class="text-sm text-stone-500 mt-0.5">Select which Gemini model to use for generation</p>
					</div>
							...
							>
								<div class="flex items-center justify-between">
									<div class="flex items-center gap-3">
										...
										<div>
											<div class="font-medium text-stone-900 text-sm">{model.name}</div>
											<div class="text-xs text-stone-500">{model.description}</div>
										</div>
									</div>
									<code class="text-[10px] text-stone-400 font-mono">{model.id}</code>
								</div>
							</button>
						))}
					</div>
